...
(function () {
    ...
    $(".add-btn").on("click", function (e) {
        const addUrl = $(this).data('url');
        const drawerTitle = $(this).data('drawer-title');
        if (drawerTitle) $('#offcanvasRightLabel1').text(drawerTitle);
        $.ajax({
            url: addUrl,
            type: 'GET',
            success: function (res) {
                if (res.html) {
                    $('#x-drawer-content').html(res.html);
                    loadFormValidation();
                    reloadScript('js/plugins/flatpickr.js');
                }
            },
        });
    })
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updatePrices() {
        ...
        prices.forEach(price => {
            price.textContent = checkbox.checked ? price.getAttribute("data-yearly") : price.getAttribute("data-monthly");
        });
    }
...
// My function
// Hàm reloadScript để tải lại script
function reloadScript(src) {
    const oldScript = document.querySelector(`script[src="${src}"]`);
    if (oldScript) oldScript.remove(); // xóa bản cũ nếu có

    const script = document.createElement('script');
    script.src = src + '?_=' + Date.now(); // thêm timestamp để tránh cache
